...
function buildClass(config, deps, json, guessedName = null) {
    const { className, properties } = getClassInfo(json, guessedName ?? config.className, config,deps);
    let classContent = buildProperties(properties, config) +
...
function buildArraySerialization(properties, config, className) {
    ...
    return buildFromArray(properties, config, className)
...
function buildFromArray(properties, { typedMethods }, className) {
    ...
    properties.forEach((property, i) => {
        ...
        if (property.type === "array") {
            ...
            content += indent(`},$data["${property.originalName}"])${isLast ? "" : ","}`, 3)
            result += content + "\n";
            return;
        }
        content = `${property.subtype}::fromArray($data["${property.originalName}"])${isLast ? "" : ","}`;
        result += indent(content + "\n", 3);
        return;
    });
    result += indent(");\n", 2)
...
function buildToArray(properties, { typedMethods }, className) {
    ...
    properties.forEach((property, i) => {
        ...
        if (property.type === "array") {
            content += indent(`"${property.originalName}"=>array_map(function($item){\n`, 3);
            content += indent(`return $item->toArray();\n`, 4);
            content += indent(`},$this->${property.name})${isLast ? "" : ","}`, 3)
            result += content + "\n";
            return;
        }
        content = `"${property.originalName}"=>$this->${property.name}->toArray()${isLast ? "" : ","}`;
        result += indent(content + "\n", 3);
        return;
    });
    result += indent("];\n", 2)
